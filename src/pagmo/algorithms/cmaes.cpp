...
cmaes::cmaes(unsigned gen, double cc, double cs, double c1, double cmu, double sigma0, double ftol,
    ...
    if (((cc < 0.) || (cc > 1.)) && !(cc == -1)) {
        pagmo_throw(std::invalid_argument, "cc must be in [0,1] or -1 if its value has to be "
                                           "initialized automatically, a value of " +
                                               std::to_string(cc) + " was detected");
    }
    if (((cs < 0.) || (cs > 1.)) && !(cs == -1)) {
        pagmo_throw(std::invalid_argument, "cs needs to be in [0,1] or -1 if its value has to be "
                                           "initialized automatically, a value of " +
                                               std::to_string(cs) + " was detected");
    }
    if (((c1 < 0.) || (c1 > 1.)) && !(c1 == -1)) {
        pagmo_throw(std::invalid_argument, "c1 needs to be in [0,1] or -1 if its value has to be "
                                           "initialized automatically, a value of " +
                                               std::to_string(c1) + " was detected");
    }
    if (((cmu < 0.) || (cmu > 1.)) && !(cmu == -1)) {
        pagmo_throw(std::invalid_argument, "cmu needs to be in [0,1] or -1 if its value has to be "
                                           "initialized automatically, a value of " +
...
population cmaes::evolve(population pop) const {
    ...
    std::normal_distribution<double> normally_distributed_number(
        0., 1.); // to generate a normally distributed number        // Setting coefficients for
                 // Selection
    Eigen::VectorXd weights(_(mu));
    for (decltype(weights.rows()) i = 0; i < weights.rows(); ++i) {
        weights(i) =
            std::log(static_cast<double>(mu) + 0.5) - std::log(static_cast<double>(i) + 1.);
    }
    ...
    if (cmu == -1) {
        cmu = 2. * (mueff - 2. + 1. / mueff) /
              ((N + 2.) * (N + 2.) + mueff); // and for rank-mu update
    }

    double damps = 1. + 2. * std::max(0., std::sqrt((mueff - 1.) / (N + 1.)) - 1.) +
                   cs; // damping coefficient for sigma
    double chiN =
        std::sqrt(N) *
        (1. - 1. / (4. * N) + 1. / (21. * N * N)); // expectation of ||N(0,I)|| == norm(randn(N,1))
    ...
    // If the algorithm is called for the first time on this problem dimension / pop size or if
    // m_memory is false we erease the memory of past calls
    if ((newpop.size() != lam) || (static_cast<unsigned>(newpop[0].rows()) != dim) ||
        (m_memory == false)) {
        sigma = m_sigma0;
        mean.resize(_(dim));
    ...
    for (decltype(m_gen) gen = 1u; gen <= m_gen; ++gen) {
        ...
        // 1bis - Logs and prints (verbosity modes > 1: a line is added every m_verbosity
        // generations)
        if (m_verbosity > 0u) {
        ...
        std::sort(best_idx.begin(), best_idx.end(),
                  [&pop](population::size_type idx1, population::size_type idx2) {
        ...
        ps = (1. - cs) * ps +
             std::sqrt(cs * (2. - cs) * mueff) * invsqrtC * (mean - meanold) / sigma;
        double hsig = 0.;
        hsig = (ps.squaredNorm() / N /
                (1. - std::pow((1. - cs), (2. * static_cast<double>(counteval) /
                                           static_cast<double>(lam))))) < (2. + 4. / (N + 1.));
        ...
        if (static_cast<double>(counteval - eigeneval) >
            ...
            if (es.info() == Eigen::Success) {
                ...
                for (decltype(dim) j = 0u; j < dim; ++j) {
                    D(_(j), _(j)) = std::sqrt(
                        std::max(1e-20, D(_(j), _(j)))); // D contains standard deviations now
                }
                for (decltype(dim) j = 0u; j < dim; ++j) {
 ...
 */
template <typename Archive> void cmaes::serialize(Archive& ar, unsigned) {
    detail::archive(ar, m_gen, m_cc, m_cs, m_c1, m_cmu, m_sigma0, m_ftol, m_xtol, m_memory,
                    m_force_bounds, sigma, mean, variation, newpop, B, D, C, invsqrtC, pc, ps,
                    counteval, eigeneval, m_e, m_seed, m_verbosity, m_log);
}
